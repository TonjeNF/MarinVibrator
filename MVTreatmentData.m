function MVTreatmentData(Tmeta_i,calibrationfactor,tempdir)
...
for j=1:length(Tmeta_i.Hydrophone)
    tmpfil = fullfile(tempdir,['Block',num2str(Tmeta_i.BlockNo),'_Treat_',Tmeta_i.Treatment,'_Hydr',num2str(j),'_' , Tmeta_i.Hydrophone(j).Location, '.mat']);
       tmpfil2 = fullfile(tempdir,['Block',num2str(Tmeta_i.BlockNo),'_Treat_',Tmeta_i.Treatment,'_Hydr',num2str(j),'_' , Tmeta_i.Hydrophone(j).Location, '.png']);
    if ~exist(tmpfil)
        ...
        if ~isempty(ind) % Check if there are missing times for the deployment
            ...
            cal = calibrationfactor(Tmeta_i.Hydrophone(j).Dmeta_index,1); %brukar kalibrering før og ikkje etter
            ...
            Dat.Time = Dat.Time(1:k2);
             ...
             f = figure('visible', 'off');
               plot(Dat.Time/60,detrend(Dat.Pressure))
xlabel('minutes')
ylabel('Pa')
 title(([datestr(Dat.T0) ' to ' datestr(Dat.T1)]))
        print([tmpfil2],'-dpng')
        close(f)
            ...
            warning(['No data for Block',num2str(Tmeta_i.BlockNo),' Treatment',num2str(Tmeta_i.TreatmentNo),' Hydrophone',num2str(j), Tmeta_i.Hydrophone(j).Location,])
        end
    end
end
end